...
export default function EventSearchBar() {
  ...
  const handleSearchEvent: FormEventHandler<HTMLDivElement> | undefined = (
    event
  ) => {
      ...
      <HStack as="form" onSubmit={handleSearchEvent} spacing={2}>
        <FormControl w="250px" mr={0} isRequired>
          <Select placeholder="Select your country">
            <option defaultValue={"United States"} value="US">
              United States
            </option>
          </Select>
        ...
        <FormControl w="250px" mr={0} isRequired>
          <Select placeholder="Select your state">
            {usa.states.map((state) => (
              <option key={state.abbreviation} value={state.name.toLowerCase()}>
