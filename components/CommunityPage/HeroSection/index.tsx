...
export default function HeroSection({logoImageHash, description, artworkHash, price, name}:Props){
    ...
    return(
        <Flex mb={9}  w='100%' position={"relative"} direction='column'>
            ...
            <Flex mb={'2rem'} alignItems={['flex-start','flex-start','center','center']} direction={['column','column']}  justifyContent={'space-between'}  mx='5' > 
                <Flex alignItems={'center'} direction={['column','row','row']}  w={'100%'} my={'2rem'}>
                    <Flex width={'100%'}>
                        <Text textStyle={'h3'} color={'accent.200'} as='h1'>${(price/100).toLocaleString()}</Text>
                    </Flex>
                    {/* share buttons */} 
                    <HStack mt={[5,5,0]}  spacing={3}>
                ...
                <Box mb={9} w={'100%'} mt={2}> 
                    <Text color={'white'} mb={'1rem'} lineHeight={[1.2,1.3]}  textStyle={['h2','h2','h1']} maxW={'800px'} as='h2' size='lg'>{name}</Text>
                    <Text  textStyle={'body'} maxW={'850px'} layerStyle={'mediumPop'}>{description}</Text>
                </Box>
              
            </Flex>
            <Flex mx={5}  w='100' mb='1rem' direction={'column'}> 
